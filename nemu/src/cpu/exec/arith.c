...
make_EHelper(add) {
  ...
  rtl_sltu(&t1,&t0,&id_dest->val);//若结果小于加数，说明溢出
  rtl_set_CF(&t1);
  rtl_xor(&t2,&id_dest->val,&id_src->val);//先看两个加数的符号是否同号
  rtl_msb(&t3,&t2,id_dest->width);
  rtl_xor(&t2,&id_dest->val,&t0);//看加数和结果是否异号
...
//  Log("id_dest->val:%x - id_src->val:%x",id_dest->val,id_src->val);
  rtl_sub(&t2,&id_dest->val,&id_src->val);//t2现在存减的值
  rtl_update_ZFSF(&t2,id_dest->width);//更新ZF和SF位 
  rtl_sltu(&t0,&id_dest->val,&t2);//若t0=1，则说明减数大于被减数
  rtl_set_CF(&t0);//设置CF位
  operand_write(id_dest,&t2);//写回

/*  rtl_xor(&t3,&id_dest->val,&id_src->val);//被减数和减数
  rtl_msb(&t0,&t3,id_dest->width);
  rtl_xor(&t3,&id_dest->val,&t2);//被减数和结果 
...
make_EHelper(cmp) {//与sub类似，只是不需要写回
  rtl_sub(&t2,&id_dest->val,&id_src->val);
 // Log("t2=%x    id_dest->val=%x    %x")
