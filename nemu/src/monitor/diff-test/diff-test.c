...
void difftest_step(uint32_t eip) {
  ...
  if(r.eax!=cpu.eax) {diff=true;Log("r.eax:%x  cpu.eax:%x",r.eax,cpu.eax);}
  else 
	  if(r.ecx!=cpu.ecx) {diff=true;Log("r.ecx:%x  cpu.ecx:%x",r.ecx,cpu.ecx);}
	  else if(r.edx!=cpu.edx) {diff=true;Log("r.edx:%x  cpu.edx:%x",r.edx,cpu.edx);}
	  else if(r.ebx!=cpu.ebx) {diff=true;Log("r.ebx:%x  cpu.ebx:%x",r.ebx,cpu.ebx);}
	  else if(r.esp!=cpu.esp) {diff=true;Log("r.esp:%x  cpu.esp:%x",r.esp,cpu.esp);}
	  else if(r.ebp!=cpu.ebp) {diff=true;Log("r.ebp:%x  cpu.ebp:%x",r.ebp,cpu.ebp);}
	  else if(r.esi!=cpu.esi) {diff=true;Log("r.esi:%x cpu.esi:%x",r.esi,cpu.esi);}
	  else if(r.edi!=cpu.edi) {diff=true;Log("r.edi:%x  cpu.edi:%x",r.edi,cpu.edi);}
	  else if(r.eip!=cpu.eip) {diff=true;Log("r.eip:%x  cpu.eip:%x",r.eip,cpu.eip);}
  if (diff) {
    nemu_state = NEMU_END;
  }
}
