...
static inline void rtl_push(const rtlreg_t* src1) {
  ...
  rtl_sm(&cpu.esp,4,src1);//esp是三十二位寄存器，因此len是4
}

static inline void rtl_pop(rtlreg_t* dest) {
  // dest <- M[esp]
  // esp <- esp + 4
  rtl_lm(dest,&cpu.esp,4);//读取M[esp]的内容
...
static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  ...
  switch(width)
  {
	case 1:	ZF=(*result&0xff)|0;
		break;
	case 2: ZF=(*result&0xffff)|0;
		break;
	case 4: ZF=(*result&0xffffffff)|0;
		break;
	default:
		ZF=0;
  }
...
static inline void rtl_update_SF(const rtlreg_t* result, int width) {
  ...
  int W=width*8;
  cpu.SF=(*result>>(W-1))&0x1;//负号为1，否则为0
}
